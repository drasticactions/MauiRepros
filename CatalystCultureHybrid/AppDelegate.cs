...
namespace CatalystCultureHybrid;
...
public class AppDelegate : UIApplicationDelegate {
	...
	public override bool FinishedLaunching (UIApplication application, NSDictionary launchOptions)
	{
		...
		var vc = new StringListTableViewController ();
...
public class StringListTableViewController : UITableViewController
{
    ...
    public override void ViewDidLoad()
    {
        ...
        TableView.RegisterClassForCellReuse(typeof(UITableViewCell), "StringCell");

        // Set the data source and delegate to this view controller
        TableView.Source = new StringListDataSource(stringList);
    }
}

public class StringListDataSource : UITableViewSource
{
    private List<string> items;
    private string cellIdentifier = "StringCell";

    public StringListDataSource(List<string> items)
    {
        this.items = items;
    }
...
public enum MauiLangLanguages : uint
{
...
public class MauiLangLanguage
{
    ...
    public MauiLangLanguage(MauiLangLanguages? language)
    {
        ...
        {
            var code = GetCode(language!.Value);
        ...
        // print culture info stats
        Console.WriteLine($"Name: {this.CultureInfo.ToString()} - {CultureInfo.Name} - {CultureInfo.NativeName} - {CultureInfo.DisplayName} - {CultureInfo.EnglishName}");
    }
    ...
    public static IReadOnlyList<MauiLangLanguage> GenerateMauiLangLangauages()
    {
        ...
        foreach (MauiLangLanguages value in Enum.GetValues(typeof(MauiLangLanguages)))
        {
            list.Add(new MauiLangLanguage(value));
        }

        var orderedList = list.OrderBy(n => n.Language);
        var newList = orderedList.ToList();
        newList.Insert(0, new MauiLangLanguage());
        return newList.AsReadOnly();
    }

    [SkipLocalsInit]
    public static string GetCode(MauiLangLanguages lang)
    {
        unsafe
        {
